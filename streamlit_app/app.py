...
def main() -> None:
    ...
    with st.spinner("Generating predictions..."):
        ...
        df_filtered["pred_mean"] = (
            bundle_mean[1]
            .predict(df_filtered[config.NUMERICAL_COLUMNS + config.CATEGORICAL_COLUMNS])
            .round(0)
        )
        df_filtered["pred_90"] = (
            bundle_90[1]
    ...
    df_complaint_summary = (
        df_filtered.groupby("complaint_family")[value_col_list].sum().reset_index()
    )
    df_complaint_summary = df_complaint_summary.sort_values("y", ascending=False)
    col1, col2, col3, col4 = st.columns(4)

    selected_summary = df_complaint_summary[
        df_complaint_summary["complaint_family"] == complaint_family
    ]
    ...
    **{pd.to_datetime(week).strftime('%B %d, %Y')}**
    ...
    st.markdown("---")
    st.caption(
        f"Data source: NYC 311 Service Requests | Model: LightGBM ({config.MODEL_TIMESTAMP})"
    )
